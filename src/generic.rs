...
impl Family {
        ...
        for attr in message.attributes {
            match AttributeId::from(attr.identifier) {
                ...
                AttributeId::MulticastGroups => {
                    let (_, mcs_attributes) = core::Attribute::unpack_all(&attr.as_bytes());
    ...
    pub fn from_name(socket: &mut core::Socket, name: &str) -> Result<Family> {
        {
            let mut tx_msg = Message::new(
                FamilyId::Control,
                Command::GetFamily,
                MessageMode::Acknowledge,
            );
            tx_msg.attributes.push(Attribute::new_string_with_nul(
                AttributeId::FamilyName,
                name,
            ));
    ...
    pub fn from_id<ID: Into<u16>>(socket: &mut core::Socket, id: ID) -> Result<Family> {
        let id = id.into().clone();
        {
            ...
            tx_msg
                .attributes
                .push(Attribute::new(AttributeId::FamilyId, id));
            socket.send_message(&tx_msg)?;
        }
        loop {
            let messages = socket.receive_messages()?;
...
mod tests {
    ...
    fn check_family_ids() {
        ...
        assert_eq!(
            u16::from(FamilyId::VirtualFileSystemDiskQuota),
    ...
    #[test]
    fn check_commands() {
        assert_eq!(u8::from(Command::Unspecified), libc::CTRL_CMD_UNSPEC as u8);
        assert_eq!(u8::from(Command::NewFamily), libc::CTRL_CMD_NEWFAMILY as u8);
        assert_eq!(u8::from(Command::DelFamily), libc::CTRL_CMD_DELFAMILY as u8);
        assert_eq!(u8::from(Command::GetFamily), libc::CTRL_CMD_GETFAMILY as u8);
        ...
        assert_eq!(
            u8::from(Command::NewMulticastGroup),
            libc::CTRL_CMD_NEWMCAST_GRP as u8
        );
        assert_eq!(
            u8::from(Command::DelMulticastGroup),
            libc::CTRL_CMD_DELMCAST_GRP as u8
        );
        assert_eq!(
            u8::from(Command::GetMulticastGroup),
    ...
    fn check_attributes() {
        assert_eq!(
            u16::from(AttributeId::Unspecified),
            libc::CTRL_ATTR_UNSPEC as u16
        );
        assert_eq!(
            u16::from(AttributeId::FamilyId),
            libc::CTRL_ATTR_FAMILY_ID as u16
        );
        assert_eq!(
            u16::from(AttributeId::FamilyName),
            libc::CTRL_ATTR_FAMILY_NAME as u16
        );
        assert_eq!(
            u16::from(AttributeId::Version),
            libc::CTRL_ATTR_VERSION as u16
        );
        assert_eq!(
            u16::from(AttributeId::HeaderSize),
            libc::CTRL_ATTR_HDRSIZE as u16
        );
        assert_eq!(
            u16::from(AttributeId::MaximumAttributes),
            libc::CTRL_ATTR_MAXATTR as u16
        );
        assert_eq!(
            u16::from(AttributeId::Operations),
            libc::CTRL_ATTR_OPS as u16
        );
        assert_eq!(
            u16::from(AttributeId::MulticastGroups),
    ...
    fn check_operation_attributes() {
        assert_eq!(
            u16::from(OperationAttributeId::Unspecified),
            libc::CTRL_ATTR_OP_UNSPEC as u16
        );
        assert_eq!(
            u16::from(OperationAttributeId::Id),
            libc::CTRL_ATTR_OP_ID as u16
        );
        assert_eq!(
            u16::from(OperationAttributeId::Flags),
    ...
    fn check_multicast_attributes() {
        assert_eq!(
            u16::from(MulticastAttributeId::Unspecified),
            libc::CTRL_ATTR_MCAST_GRP_UNSPEC as u16
        );
        assert_eq!(
            u16::from(MulticastAttributeId::Name),
            libc::CTRL_ATTR_MCAST_GRP_NAME as u16
        );
        assert_eq!(
            u16::from(MulticastAttributeId::Id),
